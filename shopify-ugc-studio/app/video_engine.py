...
def _draw_text_panel(frame: Image.Image, hook: str, subtitle: str, cta: str, progress: float) -> Image.Image:
    ...
    for line in hook_lines:
        box = draw.textbbox((0, 0), line, font=hook_font); tw = box[2] - box[0]
        draw.text(((w - tw) / 2, y), line, font=hook_font, fill="white"); y += 58
    ...
    for line in sub_lines:
        box = draw.textbbox((0, 0), line, font=sub_font); tw = box[2] - box[0]
        draw.text(((w - tw) / 2, y), line, font=sub_font, fill="white"); y += 44
    if progress > 0.76:
        cta_box = (110, h - 112, w - 110, h - 46)
        draw.rounded_rectangle(cta_box, radius=28, fill=(255, 255, 255, 235))
        box = draw.textbbox((0, 0), cta, font=cta_font); tw = box[2] - box[0]
        draw.text(((w - tw) / 2, h - 99), cta, font=cta_font, fill=(10, 10, 10))
    draw.rectangle((0, h - 10, int(w * progress), h), fill=(255, 255, 255, 230))
    return frame


def render_video(product: dict, concept: dict, output_dir: Path, duration: int = 18, fps: int = 24, with_voice: bool = True) -> Path:
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            frame = _cover(base, (720, 1280), 1.0 + 0.055 * local)
            frame = ImageEnhance.Contrast(frame).enhance(1.03)
            frame = _draw_text_panel(frame, concept.get("hook", ""), scene_texts[scene_idx], concept.get("cta", "Scopri di più"), progress)
    ...
    if with_voice and _tts_windows(concept.get("script", ""), audio):
